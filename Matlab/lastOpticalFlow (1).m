...
lineX2 = 100;       %y-position of ROI
lineY1 = 100;    %x-Position of ROI 
lineY2 = 240 ;     %x-Position of ROI
...
while ~isDone(hVidReader)  % Stop when end of file is reached
          ...
          if(Idx(i))          %if they lies in ROI
              ...
              if(~isempty(img))   % If croped image is not empty then 
                    ...
                    if (d(2)> 80)& (d(2) < 110)   
                        ...
                        if (b(2) - d(2)) <= 0   % Identify that the detected vehical is same or not if yes.. 
                   ...
                   if (d(2)>110) & (d(2)<180)
                       ...
                       % insert Annotation
                          ...
                          if strcmp('Alto',text)
                              ...
                              name = sprintf('C:\\Users\\lenovo\\Desktop\\matlab\\Ajay\\cAlto\\alto%d',count)
                              imwrite(img,name,'jpg');
                          end
                          if strcmp('city',text)
                               %location = fullfile('curLocation','cCity');
                               name = sprintf('C:\\Users\\lenovo\\Desktop\\matlab\\Ajay\\cCity\\City%d',count)
                               imwrite(img,name,'jpg');
                               
                          end
                          if strcmp ('Swift',text)
                              %location = fullfile('curLocation','cSuzuki');
                              name = sprintf('C:\\Users\\lenovo\\Desktop\\matlab\\Ajay\\cSwift\\Swift%d',count)
                              imwrite(img,name,'jpg');
                          end    
                          end
                   end
                   if d(2) > 170   % if object goes out of our ROI 
    ...
    %Draw the ROI 
    %y2(100:100,5:140,:) = 1;   % The white line.
    %y2(5:7,5:140,:)   = 1;   % The white line.
    %y2(5:100,5:7,:)  = 1;   % The white line.
    %y2(5:100,140:142,:)  = 1;   % The white line.
    y2(1:35,1:60,:) = 0;   % Background for displaying count
    result = step(htextins, y2,int32(nbbox) );
    %% show the result .
    subplot(1,3,2);
    imshow(result);
